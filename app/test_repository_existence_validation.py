#!/usr/bin/env python3
"""
Validation script for Task 4.1.2: Repository existence check logic implementation.

This script validates the enhanced repository existence check functionality
without relying on pytest import paths.
"""
...
try:
    ...
    try:
        from app.services.repository_cache_manager import RepositoryCacheManager
        from app.core.exceptions import RepositoryError
        from app.core.structured_logging import LogStatus
...
def test_repository_existence_check():
    ...
    print("🚀 Starting Repository Existence Check Validation (Task 4.1.2)\n")
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            manager = RepositoryCacheManager(correlation_id="test_validation_4_1_2")
            
            print("✅ Test 1: Repository existence check with cache miss")
            ...
            for malicious_url in malicious_urls:
                try:
                    ...
                    assert False, f"Security validation failed for: {malicious_url}"
                except RepositoryError:
                    pass  # Expected
            
            print(f"   ✓ Security validation passed for {len(malicious_urls)} malicious URLs")
            ...
            print("\n✅ Test 6: Integration with workflow patterns")
            workflow_url = "https://github.com/devteam/workflow-integration.git"
            
            # Simulate workflow integration scenario
            # 1. Check existence (cache miss)
            result1 = manager.check_repository_existence(
                repository_url=workflow_url,
                project_id="workflow_project",
                execution_id="workflow_execution"
            )
            assert result1['validation_status'] == "cache_miss"
            
            # 2. Initialize repository (create cache)
            cache_path = manager.create_cache_directory(
                repository_url=workflow_url,
                project_id="workflow_project",
                execution_id="workflow_execution"
            )
            
            # 3. Check existence again (cache hit)
            result2 = manager.check_repository_existence(
                repository_url=workflow_url,
                project_id="workflow_project",
                execution_id="workflow_execution"
            )
            ...
            print(f"   ✓ Workflow integration pattern validated")
            print(f"   ✓ Cache miss → Initialize → Cache hit flow working")
