#!/usr/bin/env python3
"""
Task 8.1.2: API Field Alignment Validation Test
...
class FieldAlignmentValidator:
    ...
    async def call_status_api(self, project_id: str = "test-project") -> Dict[str, Any]:
        """Call the DevTeam automation status API endpoint"""
        try:
            start_time = time.time()
            
            response = await self.client.get(
                f"{self.base_url}/api/v1/devteam-automation/{project_id}/status"
            )
            
            end_time = time.time()
            response_time_ms = (end_time - start_time) * 1000
            
            logger.info(f"API response time: {response_time_ms:.2f}ms")
    ...
    def extract_api_response_fields_from_data(self, api_data: Dict[str, Any]) -> Set[str]:
        """Extract all fields from actual API response data recursively"""
        def extract_fields_recursive(obj: Any, prefix: str = "") -> Set[str]:
            fields = set()
            ...
            if isinstance(obj, dict):
                for key, value in obj.items():
                    ...
                    nested_fields = extract_fields_recursive(value, field_name)
                    fields.update(nested_fields)
                    
            elif isinstance(obj, list) and obj:
                # For lists, analyze the first item structure
                nested_fields = extract_fields_recursive(obj[0], prefix)
                fields.update(nested_fields)
                
            return fields
        
        return extract_fields_recursive(api_data)
    ...
    async def validate_performance_requirement(self, project_id: str = "test-project", iterations: int = 5) -> Dict[str, Any]:
        """Validate API response time meets ≤200ms requirement"""
        response_times = []
        
        for i in range(iterations):
            result = await self.call_status_api(project_id)
        ...
        if not response_times:
            return {
                "success": False,
                "error": "No successful API calls to measure performance"
            }
        ...
        meets_requirement = max_response_time <= 200.0
    ...
    async def run_comprehensive_validation(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 7. Performance validation (if server is running)
            logger.info("Step 7: Validating API performance requirements")
            try:
                performance_validation = await self.validate_performance_requirement()
                results["validation_results"]["performance_validation"] = performance_validation
            except Exception as e:
                results["validation_results"]["performance_validation"] = {
                    "success": False,
                    "error": f"Performance validation failed (server may not be running): {e}",
                    "note": "This is expected if the server is not running"
                }
            
            # 8. Overall assessment
            logger.info("Step 8: Generating overall assessment")
    ...
    def generate_overall_assessment(self, validation_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if performance_validation.get("success") and not performance_validation.get("meets_200ms_requirement", True):
                assessment["warnings"].append("API response time exceeds 200ms requirement")
            ...
            if performance_validation.get("meets_200ms_requirement"):
                scores.append(100)
            elif performance_validation.get("success"):
                scores.append(50)
            ...
            if not performance_validation.get("meets_200ms_requirement", True):
                assessment["recommendations"].append("Optimize API response time to meet ≤200ms requirement")
