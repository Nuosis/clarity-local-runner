...
class TestDevTeamStatusEndpointComprehensive:
    """Comprehensive test suite for DevTeam Automation Status Endpoint."""
    ...
    def expected_response_format(self):
        """Expected response format according to ADD Section 5.2."""
        return {
            "success": True,
            "data": {
                "status": "running",
                ...
                "execution_id": "exec_12345678-1234-1234-1234-123456789012",
                "branch": "task/1-1-1-add-devteam-enabled-flag",
                "started_at": "2025-01-14T18:25:00",
                "updated_at": "2025-01-14T18:30:00"
            },
    ...
    @pytest.mark.asyncio
    async def test_get_devteam_automation_status_success(self, mock_session, sample_status_projection):
        ...
        with patch('api.v1.endpoints.devteam_automation.get_status_projection_service') as mock_service_factory:
            ...
            result = await get_devteam_automation_status(
                project_id="customer-123/project-abc",
                session=mock_session
            )
            
            assert isinstance(result, DevTeamStatusSuccessResponse)
            assert result.success is True
            assert result.data.status == "running"
            assert result.data.progress == 45.2
            assert result.data.current_task == "1.1.1"
            assert result.data.execution_id == "exec_12345678-1234-1234-1234-123456789012"

    @pytest.mark.asyncio
    async def test_get_devteam_automation_status_not_found(self, mock_session):
        """Test status retrieval when project not found."""
        with patch('api.v1.endpoints.devteam_automation.get_status_projection_service') as mock_service_factory:
            mock_service = Mock(spec=StatusProjectionService)
            ...
            with pytest.raises(HTTPException) as exc_info:
                await get_devteam_automation_status(
                    project_id="customer-nonexistent/project-missing",
                    session=mock_session
                )
            
            assert exc_info.value.status_code == 404
            assert "No automation status found" in str(exc_info.value.detail)

    @pytest.mark.asyncio
    async def test_get_devteam_automation_status_validation_error(self, mock_session):
        """Test status retrieval with invalid project ID."""
        invalid_project_ids = [
            "",  # Empty
...
class TestDevTeamStatusEndpointPerformance:
    """Performance-focused tests for the status endpoint."""
